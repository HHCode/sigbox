/* Copyright (C) 2011-2012 IC Nexus Co., LTD.
 *
 * TCC8900 DVK10 watchdog timer test program in user space
 *  
 * 2011/09/20: 
 *        1. Initial.
 *
...
int main(int argc, char **argv)
{
	...
	printf("Last reboot was %sinitiated by watchdog timer.\n",
		(boot_status & WDIOF_CARDRESET) ? "" : "not ");

	if (to) {
		//printf("WDIOC_SETTIMEOUT=%d", WDIOC_SETTIMEOUT);
		if (ioctl(fd, WDIOC_SETTIMEOUT, &to)) {
			perror("ioctl");
			goto error_exit;
		}
	} else {
		//printf("WDIOC_GETTIMEOUT=%d", WDIOC_GETTIMEOUT);
